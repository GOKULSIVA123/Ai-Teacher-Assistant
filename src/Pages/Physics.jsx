...
import physicsQuestions from "./physics.json";
...
function Physics() {
  ...
  const questions = physicsQuestions.questions;
  ...
  function handleAnswer(choice) {
    ...
    setResult((prev) => ({
      score: isCorrect ? prev.score + physicsQuestions.perQuestionScore : prev.score,
  ...
  function nextQuestion() {
    if (selectedAnswer === null) return; // Require answer selection
  ...
  return (
    <div className="w-full h-screen flex flex-col justify-center items-center bg-purple-300 p-4">
      {!showResult ? (
        <div className="w-full max-w-xl bg-white rounded-lg p-6 shadow-lg">
          <h2 className="text-xl font-semibold mb-4">
            Physics Quiz ({activeQuestion + 1}/{questions.length})
          </h2>
